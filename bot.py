    ...
    if usr_cmd == "/start":
        ...
        await cmd.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "Support Group", url=f"https://t.me/{Config.USERNAME_GROUP}"
                        ),
                        InlineKeyboardButton(
                            "Bots Channel",
                            url=f"https://t.me/{Config.USERNAME_CHANNEL}",
                        ),
                    ],
                    [
        ...
        try:
            ...
            if Config.FORWARD_AS_COPY is True:
                send_stored_file = await bot.copy_message(
                    chat_id=cmd.from_user.id,
                    from_chat_id=Config.DB_CHANNEL,
                    message_id=file_id,
                )
            elif Config.FORWARD_AS_COPY is False:
                send_stored_file = await bot.forward_messages(
                    chat_id=cmd.from_user.id,
                    from_chat_id=Config.DB_CHANNEL,
                    message_ids=file_id,
                )
            await send_stored_file.reply_text(
                f"**Ini Tautan yang Dapat Dibagikan dari file ini:** https://t.me/{Config.BOT_USERNAME}?start=bot_{file_id}\n\n__Untuk Mengambil File Tersimpan, buka saja tautannya!__",
                disable_web_page_preview=True,
                quote=True,
            )
...
async def main(bot: Client, message: Message):
    ...
    if message.chat.type == "private":
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            ...
                            InlineKeyboardButton(
                                "Support Group",
                                url=f"https://t.me/{Config.USERNAME_GROUP}",
                            ),
                        ],
                    ]
                ),
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "Ban User",
                                callback_data=f"ban_user_{str(message.chat.id)}",
                            )
                        ]
                    ]
                ),
        ...
        if (
            (message.chat.id == Config.LOG_CHANNEL)
            or (message.chat.id == int(Config.UPDATES_CHANNEL))
        ...
        try:
            ...
            CH_edit = await bot.edit_message_reply_markup(
                ...
                message.message_id,
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("Get Sharable Stored Link", url=share_link)]]
                ),
            )
...
@Bot.on_message(
    filters.private & filters.command("status") & filters.user(Config.BOT_OWNER)
)
...
@Bot.on_message(
    filters.private & filters.command("unban_user") & filters.user(Config.BOT_OWNER)
)
...
@Bot.on_message(
    filters.private & filters.command("banned_users") & filters.user(Config.BOT_OWNER)
)
...
async def button(bot: Client, cmd: CallbackQuery):
    ...
    if "aboutbot" in cb_data:
        ...
        if Config.UPDATES_CHANNEL:
            ...
            try:
                user = await bot.get_chat_member(
                    int(Config.UPDATES_CHANNEL), cmd.message.chat.id
                )
        ...
        if Config.UPDATES_CHANNEL is None:
            await cmd.answer(
                "Sorry Sir, You didn't Set any Updates Channel!", show_alert=True
            )
            return
        ...
        try:
            await bot.kick_chat_member(
                chat_id=int(Config.UPDATES_CHANNEL), user_id=int(user_id)
            )
